...
Run `AWS_PROFILE=admin ./deploy-transcription-caller.sh` to build and push immutable transcription-caller and task-callback images, apply their Terraform resources and audio-processing workflow integration, wait for both Lambda updates, and run the ignored start-evaluation integration suite using `sample_071.mp3`. The upload/start-evaluation stack must already be deployed; the happy-path test waits for audio conversion, transcription, callback delivery, and terminal Step Functions success. The Modal endpoint and workspace have repository defaults and can be overridden with `TRANSCRIPTION_ENDPOINT_URL` and `MODAL_WORKSPACE_ID`. Pass `--auto-approve` for unattended Terraform applies, `--audio-file FILE` to use another fixture, or `--skip-integration-test` for deployment only. The script uses the local `admin` AWS profile and intentionally targets the transcription resources and workflow wiring so unrelated Lambda image tags are not reset by Terraform defaults.
